...
def test_1_login(client, mock_login_user):
...
def test_2_login(client, mock_login_user):
    # Mock status code of response.
    mock_login_user.return_value.status_code = 200
    mock_login_user.return_value.json.return_value = json.dumps(
        {"id": 2, "username": "servererror", "password": "test1123"})
...
def test_3_login(client, mock_login_user):
    # Mock status code of response.
    mock_login_user.return_value.status_code = 200
    mock_login_user.return_value.json.return_value = json.dumps(
        {"we": 2, "maronn": "coseAcaso", "belle": "pizza"})
